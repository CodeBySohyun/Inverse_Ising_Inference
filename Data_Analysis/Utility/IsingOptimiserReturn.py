...
class IsingOptimiser:
    def __init__(self, data, start_date=None, end_date=None, scale=None):
        """
        ...
        """
        # Check if data is a path or a DataFrame
        if isinstance(data, pd.DataFrame):
            self.df = data
        elif isinstance(data, str):
            self.df = pd.read_csv(data)
        else:
            raise ValueError("Data must be a pandas DataFrame or a filepath string.")
        
        # If start and end dates are specified, filter the DataFrame
        if start_date and end_date:
            self.df['Date'] = pd.to_datetime(self.df['Date'])
            self.df = self.df[(self.df['Date'] >= start_date) & (self.df['Date'] <= end_date)]

        self.initial_state = self.df.drop(columns=["Date"]).mean().apply(lambda x: 1 if x > 0 else -1).to_numpy()
        self.symbols = self.df.columns.tolist()[1:]
        self.X = self.df.drop(columns=["Date"]).to_numpy()
        self.blocks = np.concatenate([self.X[:scale, i] for i in range(self.X.shape[1])])
    ...
    def train(self, max_attempts=1000, display_results=None):
        ...
        if J_results:
            ...
            print(f"Optimisation completed with averaged results after {success_count} successes with {attempts_count} attempts.\n")

        else:
            print("Optimisation was unsuccessful after maximum attempts.\n")

        return self.initial_state, self.J_optimised, self.h_optimised
